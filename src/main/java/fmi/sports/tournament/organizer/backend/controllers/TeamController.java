...
public class TeamController {
  ...
  public ResponseEntity<TeamResponse> getById(@PathVariable("id") Long teamId) {
    ...
    TeamResponse response =
        TeamResponse.fromDTO(dto).responseResult(ResponseResult.SUCCESSFULLY_FOUND).build();
  ...
  public ResponseEntity<TeamResponse> update(@Valid @RequestBody TeamDTO updatedTeams) {
    ...
    return ResponseEntity.ok(
        TeamResponse.fromDTO(updated).responseResult(ResponseResult.SUCCESSFULLY_UPDATED).build());
  }
  ...
  public ResponseEntity<TeamResponse> deleteById(@PathVariable("id") Long teamId) {
    ...
    return ResponseEntity.ok(
        TeamResponse.fromDTO(teamDTO).responseResult(ResponseResult.SUCCESSFULLY_DELETED).build());
  }

  @PostMapping("/{teamId}/participants")
  public ResponseEntity<String> registerUserForTeam(
      @RequestHeader("Authorization") String authorizationHeader,
      @RequestBody ParticipantRegisterDTO participantRegisterDTO,
  ...
  public ResponseEntity<TeamResponse> removeUserFromTeam(
      @RequestHeader("Authorization") String authorizationHeader,
      @PathVariable("teamId") Long teamId) {
    String token =
        authorizationHeader.startsWith("Bearer ")
    ...
    return ResponseEntity.ok(
        TeamResponse.builder()
            .message(
                String.format(
                    "User with id %d successfully removed from team with id %d", userId, teamId))
  ...
  public List<ParticipantResponse> getAllParticipantsForTeam(@PathVariable("teamId") Long teamId) {
    return teamService.getAllParticipantsForTeam(teamId).stream()
        .map(participantDTO -> ParticipantResponse.fromDTO(participantDTO).build())
        .collect(Collectors.toList());
  }

  @GetMapping("/{teamId}/participants/{participantId}")
  public ResponseEntity<ParticipantResponse> getParticipantForTeamById(
      @PathVariable("teamId") Long teamId, @PathVariable("participantId") Long participantId) {
    ParticipantDTO participant = teamService.getParticipantInTeamById(teamId, participantId);
    return ResponseEntity.ok()
        .body(
            ParticipantResponse.fromDTO(participant)
                .responseResult(ResponseResult.SUCCESSFULLY_FOUND)
                .build());
  }

  @PutMapping("/{teamId}/participants")
  public ResponseEntity<ParticipantResponse> updateParticipantCategory(
      @PathVariable("teamId") Long teamId,
      @RequestBody ChangeParticipantCategoryDTO participantCategoryDTO) {
    ParticipantDTO updatedParticipantDTO =
        teamService.updateParticipantCategory(teamId, participantCategoryDTO);
    return ResponseEntity.ok(
        ParticipantResponse.fromDTO(updatedParticipantDTO)
            .responseResult(ResponseResult.SUCCESSFULLY_UPDATED)
            .build());
  }

  @PostMapping("/{teamId}/followers/{userId}")
  public void subscribeForTeam(
  ...
  public void unsubscribeFromTeam(
      @PathVariable("userId") Long userId, @PathVariable("teamId") Long teamId) {
  ...
  public List<UserResponse> getAllFollowersForTeam(@PathVariable("teamId") Long teamId) {
    return teamService.getAllFollowers(teamId).stream()
