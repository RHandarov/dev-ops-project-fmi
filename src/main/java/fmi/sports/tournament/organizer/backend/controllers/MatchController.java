...
public class MatchController {
  ...
  @PostMapping
  public ResponseEntity<MatchResponse> schedule(
      @PathVariable("tId") Long tournamentId, @Valid @RequestBody MatchDTO newMatch) {
    newMatch.setTournamentId(tournamentId);
    MatchDTO scheduledMatch = matchService.schedule(newMatch);
    return ResponseEntity.status(HttpStatus.CREATED)
        .body(
            MatchResponse.fromDTO(scheduledMatch)
  ...
  public List<MatchResponse> getAllMatchedForTournament(@PathVariable("tId") Long tournamentId) {
    return matchService.getAllForTournament(tournamentId).stream()
        .map(
            matchDTO ->
  ...
  public ResponseEntity<MatchResponse> updateMatchScore(
      @PathVariable("mId") Long matchId, @Valid @RequestBody MatchResultsDTO newScore) {
