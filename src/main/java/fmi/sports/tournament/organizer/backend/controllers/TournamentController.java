...
public class TournamentController {
  ...
  public List<TournamentResponse> getAll() {
    return tournamentService.getAll().stream()
        .map(
            tournamentDTO ->
  ...
  public ResponseEntity<TournamentResponse> create(
      @Valid @RequestBody TournamentDTO newTournament) {
  ...
  public ResponseEntity<TournamentResponse> update(
      @Valid @RequestBody TournamentDTO updatedTournament, @PathVariable long id) {
    tournamentService.updateById(updatedTournament, id);
    return ResponseEntity.ok(
        TournamentResponse.fromDTO(updatedTournament)
            .id(id)
  ...
  public ResponseEntity<TeamResponse> registerTeamForParticipation(
    ...
    return ResponseEntity.ok(
        TeamResponse.builder()
            ...
            .message(
                String.format(
                    "Team with id %d successfully registered for tournament with id %d.",
                    teamId, tournamentId))
            .build());
  }
  ...
  public ResponseEntity<TeamResponse> unregisterTeamForParticipation(
    ...
    return ResponseEntity.ok(
        TeamResponse.builder()
            ...
            .message(
                String.format(
                    "Team with id %d successfully unregistered for tournament with id %d.",
                    teamId, tournamentId))
            .build());
  }

  @GetMapping("/{tournamentId}/standings")
  public List<StandingResponse> getTournamentStandings(
      @PathVariable("tournamentId") Long tournamentId) {
    return tournamentService.getTournamentStandings(tournamentId).stream()
