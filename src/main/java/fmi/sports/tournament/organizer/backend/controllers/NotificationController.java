...
public class NotificationController {
  ...
  public List<NotificationResponse> getAllNotifications(
    ...
    return notificationService.getAllNotificationsByUserId(userId).stream()
        .map(
            notificationDTO ->
                NotificationResponse.fromDTO(notificationDTO)
  ...
  public List<NotificationResponse> getAllUnreadNotifications(
      @RequestHeader("Authorization") String authorizationHeader) {
    Long userId = getLoggedUserId(authorizationHeader);
    return notificationService.getAllUnreadNotificationsByUserId(userId).stream()
        .map(
            notificationDTO ->
                NotificationResponse.fromDTO(notificationDTO)
                    .responseResult(ResponseResult.SUCCESSFULLY_FOUND)
