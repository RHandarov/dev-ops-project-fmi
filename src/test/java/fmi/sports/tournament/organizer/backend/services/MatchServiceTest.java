...
class MatchServiceTest {
  @Mock private MatchesRepository matchRepository;
  ...
  void updateMatchResult_ShouldUpdateScoreAndStatus() {
    Team mockTeam1 =
        new Team(1L, "Team 1", "some@example.com", 250.0, 12, "some_secret", null, null, null);
    Team mockTeam2 =
        new Team(2L, "Team 2", "some@example.com", 250.0, 12, "some_secret", null, null, null);
    ...
    when(standingsRepository.findById(new StandingId(mockTournament.getId(), mockTeam1.getId())))
        .thenReturn(
            Optional.of(
                new Standing(
                    mockTournament.getId(), mockTeam1.getId(), mockTournament, mockTeam1, 2)));
